...
tricounty_911 <- bind_rows(allegany_911, cecil_911, carroll_911)
...
baltimore_911 <- baltimore_911 |> left_join(maryland_zcta, join_by(zip_code))
...
A1. Cecil with the msot, then Carroll in second and Allegany in third. 
...
A2. In both Cecil and Allegany, August saw the most 911 calls, reflecting the general observable trend that summer months, and those months that surround the season, like May or September, have the most calls. The location with the most calls across the three counties is 314 Grove Neck Rd in Cecil County, which is a drug rehabilitation center in Earleville. 

```{r}
tricounty_911 <- tricounty_911 |>
...
A3. The zip codes where the u18 population are primarily in West or Downtown Baltimore. Of those, the zip code with the most calls per capita is 21217, a West Baltimore zip code that includes Coppin State University and parts of Leakin Park. Neighborhoods there include Druid Hill Park, Bolton Hill, Harlem Park, Easterwood, Parkview/Woodbrook, Penn Hill, Reservoir Park and Mondawmin. It is a heavily Black zip code; the population is over 86% Black. It has a high percentage of renters--70%--and just 11% of the residents are retirement age. 

```{r}
baltimore_911_u18 <- baltimore_911 |>
  filter(PCT_UNDER_18 >= 75)

baltimore_911_u18_zip <- baltimore_911_u18 |>
  group_by(zip_code, POPULATION) |>
  summarise(count = n()) |>
  arrange(desc(zip_code))

baltimore_911_u18_zip <- baltimore_911_u18_zip |>
  mutate(calls_per_capita = (count/POPULATION)*1000)

baltimore_911_u18_zip |>
...
Q4: Which majority-white zip code in Baltimore has the highest rate of overdose calls? How does that number compare to the highest majority-Black zip codes?

A4: The majority-white zip code with the highest overdose call rate, 21230, would have the eight-highest among majority-Black zip codes, the worst of which has a overdose call rate nearly triple that of 21230. 

```{r}
#Which majority-white zip code in Baltimore has the highest number of overdose calls?
baltimore_911_mw <- baltimore_911 |>
  filter(PCT_WHITE_NON_HISPANIC > 50)

baltimore_911_mw_zip <- baltimore_911_mw |>
  group_by(zip_code, POPULATION) |>
  summarise(count = n()) |>
  mutate(calls_per_capita = (count/POPULATION)*1000) |>
  arrange(desc(calls_per_capita)) 
...
#How does that number compare to the highest majority-Black zip codes?
baltimore_911_mb <- baltimore_911 |>
  filter(PCT_BLACK_NON_HISPANIC > 50)

baltimore_911_mb_zip <- baltimore_911_mb |>
  group_by(zip_code, POPULATION) |>
  summarise(count = n()) |>
  mutate(calls_per_capita = (count/POPULATION)*1000) |>
  arrange(desc(calls_per_capita)) 
