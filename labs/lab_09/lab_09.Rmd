...
**A1.** https://www.datawrapper.de/_/F2NeV/ 

```{r}
md_gov22 <- read_csv("data/md_gov_county.csv")
md_gov22 <- md_gov22 |>
  clean_names() |>
  rename(write_ins22 = write_ins) |>
  mutate(total22 = cox + moore + lashar + wallace + harding + sefcik + write_ins22)

md_gov18 <- read_csv("data/md_gov_county_18.csv")
md_gov18 <- md_gov18 |>
  clean_names() |>
  rename(write_ins18 = write_ins) |>
  mutate(total18 = hogan + jealous + quinn + schlakman + write_ins18)

md_gov_counties <- left_join(md_gov18, md_gov22, by = "geoid")

md_gov_counties <- md_gov_counties |>
  select(-county.y) |>
  rename(county = county.x) |>
  mutate("Cox 2022 Vote Share" = (cox/total22)*100) |>
  mutate("Hogan 2018 Vote Share" = (hogan/total18)*100)

write_csv(md_gov_counties, "data/md_gov_counties.csv")
...
**A2.** The story here is that despite mail-in voting's explosion in popularity during teh pandemic-era election cycle, voters widely returned to the in-person, Election Day ballot boxes in 2022, just two years after Covid-19 upended traditional voting methods. https://www.datawrapper.de/_/sljoK/

```{r}
md_votetypes <- read_csv("data/md_votes_by_type.csv")

md_votetypes <- md_votetypes |>
  mutate("Early Voting" = (Early/Total)*100) |>
  mutate("Election Day Voting" = (`Election Day`/Total)*100) |>
  mutate("Mail Voting" = (Mail/Total)*100)

write_csv(md_votetypes, "data/md_votetypes.csv")
```
...
**A3.** The story is that overdose calls aren't spread equally across Montgomery County's five-largest urban centers; Germantown and Gaithersburg has especially high instances of calls. However, these are raw numbers, and not per capita rates, so I would need to do further research to see if those rate are truely disproportional, since not all zip codes have equal populations.  https://www.datawrapper.de/_/RjECh/ 

```{r}
moco_od <- read_csv("data/montgomery_2022_overdoses.csv")

moco_od_zips <- moco_od |>
  group_by(zip) |>
  summarise(Total = n()) |>
  arrange(desc(Total)) 

moco_od_zips |>
  summarise(Total_Calls = sum(Total)) 

moco_od_zips <- moco_od_zips |>
mutate(Percentage = round((Total / 659) * 100, 2))


write_csv(moco_od_zips, "data/moco_od_zips.csv")
