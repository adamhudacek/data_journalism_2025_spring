...
In the space below, write the data type for the "id" column and the "seats" column (it's in light gray, offset by \< \>). **Answer** The "id" column is a character vector (chr), which is qualitative, while the "seats" column is a double vector, which is quantitative.
...
In the space below, write the name of the business from the first row of the "name" column. **Answer** SKIPPED AS INSTRUCTED
...
**Answer** It's listing the different class terms at UMD--Fall, Winter, Spring, Summer--for each year starting in 2017, and how many classes were offered during each. 
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer** This codeblock reordered the results from the previous codeblock and sorted them from most class offerings to least class offerings. We learn that Fall 2024 had the most class offerings since data collection began. 
...
**Answer** This list, over 4,000 rows long, shows how many classes were offered in each major in each semester since the beginning of 2017. It splits up the data from the previous codeblocks into a longer, more detailed dataset. 
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer** This is the same as the second codeblock from Task 10, with an additional row added. The new row shows how many total seats were available in a given semester. The data is sorted greatest to least from the new column.
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **Answer** The new codeblock adds two columns to the previous codeblock, showing the mean and median number of seats in for each semester since the start of 2017.
...
**Answer** The data is now sorted by major, from the majors with the most total seats to the major with the least total seats. We can now see that most majors share a minimum number of seats at 0, but range wildly in their largest seat numbers.This codeblock also eliminates any sorting by data or semester, which differs from previous sorting methods. 
...
1.  Which majors have seen the largest reductions in class offerings since 2017?

2.  Did the COVID-19 pandemic affect the number of courses offered in a semester, or the number of seats in those courses?

3.  Which major has the largest mean number of seats in their classes, and has that changed since 2017?
