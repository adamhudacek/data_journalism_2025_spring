...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **Answer** Each object, or row, has an ID, two Zip Code varieties with the associated name of the town and state, an "existing" value that I'm unclear about, as well as geographic characteristics of the object, like length and area.
...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **Answer** It's a list of zip codes within Maryland, identified by the both the five-digit zip code and the extended 11-digit variety. The geodata provides a shape outline for each of the zip codes within the state.
...
**Task** Run the following code. Describe the output in the space below. **Answer** The output is a map of Maryland, separated by the state's different zip codes, taken and compiled from the md_zips dataframe and the geodata within it.
...
**Task** Run the following code. Describe the output in the space below. **Answer** A dataframe appears in below the R codespace with values for binary, source and need_compulation, which is given a value of FALSE.
...
**Task** Run the following code. Describe the output in the space below. **Answer** It appears to be a list containing the geodata and identifying information of each county in the United States, from all fifty states.
...
**Task** Run the following code. Describe the output in the space below. **Answer** This creates a new list of only Maryland counties, as Maryland is given the state code of '24' in the larger dataframe. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** The output is a map of Maryland, separated by the state's different counties, taken and compiled from the md_counties dataframe and the geodata within it.
...
**Task** Run the following code. Describe the output in the space below. **Answer** A combination of the previous two ggplot maps, this graphic overlays Maryland's zip code borders atop its county borders, although without color coordination, it's difficult to tell what's what.
...
**Task** Run the following code. Describe the output in the space below. **Answer** The dataframe lists how many foreclosures occured in each Maryland zip code in the month of March, 2023.
...
**Task** Run the following code. Describe the output in the space below. **Answer** Adds the geodata of each zip code to the list of March 2023 foreclosures in each locality. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** The code created a heatmap ggplot that highlights zip codes with the most foreclosures in March 2023.
...
**Task** Run the following code. Describe the output in the space below. **Answer** We've created two dataframes, one of the population of each Maryland county and another with the number of March 2023 foreclosures in each county.
...
**Task** Run the following code. Describe the output in the space below. **Answer** The population dataframe has been edited to include the geodata for each Maryland county.
...
**Task** Run the following code. Describe the output in the space below. **Answer** When we run these two datframes, we can see the discrepancies in the county column values. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** We've edited the population dataframe to standardize the county names so that they'll match the values in the foreclosure noticed dataframe, which will allow them to join. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** We've joined the population and foreclosure notices dataframes into a single dataframe with information, both numerical and geographical, from both.
...
**Task** Run the following code. Describe the output in the space below. **Answer** We created a new column in the joined dataframe that has a standard foreclosure rate, so that more populated counties don't receive unequal weighting in the data. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** We created another heatmap ggplot of Maryland, which is divided into counties and colored to highlight the counties with the highest foreclosure rate in March 2023.
...
**Task** Run the following code. Describe the output in the space below. **Answer** The colors of the heatmap changed to better highlight those counties with high foreclosure rates. 
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer** There aren't many distinct regional patterns, but it does appear as though south/central Maryland has higher rates than other parts of the state, although certain outliers don't align with that. 
