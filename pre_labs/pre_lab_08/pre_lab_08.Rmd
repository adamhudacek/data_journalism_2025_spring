...
**Task** Run the following code to add a month column to our dataframe using lubridate and count the number of calls for each month. In the space below, explain what each step of the code below is doing in English. Note the "label=TRUE" part in the mutate statement. **Answer** We're creating a new dataframe from the previous dataframe, and within it, we're establishing a new column, titled 'month', that is a date column but uses the month abbreviations, not the ymd format, to label the rows. We're then grouping by month and counting up the incidents in each, then arranging the months from most to least incidents. Finally, we're displaying the data.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** A large, blank box. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** I see a bar chart with dark gray bars, reaching up from an x-axis of months and judged against a y-axis of incident count, ranging from 0 to nearly 400. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** The months have been reordered from the least incidents to the most.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** The x-axis and y-axis have flipped, with the month with the highest number of incidents, August, now filling the top of the bar chart. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** The light-gray background has become the same white as the chart's borders, and the months of the y-axis have lost the black bar that used to follow them. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last version of the chart? **Answer** The aesthetics of the chart have changed, with the background taking on a new blue-gray color. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** The name of the y-axis has been simplified and a title has appeared above the data. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** I see a similar list of incident counts, but instead of months, these incidents are grouped by individual dates.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** I see a jagged line chart without any clear pattern, due to the variance in incidents between dates within the same month. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** The line chart now sits against a graphical backdrop and the x-axis is a series of unreadable week dates. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** those week dates have been improved slightly, but are still breathing down each others necks, congested and hard to read. 
