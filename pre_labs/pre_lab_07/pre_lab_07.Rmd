...
```{r echo=FALSE}
census_api_key("a053c2d5d60c9fa4a553689edbf3924153fa13b2", install=TRUE)
```
...
**Task** Run the following codeblock to create a chart showing the margin of error for each county. The current headline isn't very descriptive of the data. What should it be? **Answer** 2021 Median household income in Maryland counties
...
**Task** Let's look at the average household income for high schools that have 60% or more graduates immediately enrolling in college, and then look at those with less than 40%. Write a sentence describing the results. **Answer** Schools with high college enrollment rates, which we qualify as over 60%, have a median income of over 110 thousand dollars, far above the nearly median income of 70 thousand dollars for schools with low college enrollment rates, under 40%. 
...
**Task** What is another question you could ask of this data? **Answer** Does school size have any correlation to graduation rate, and if so, does that reflect a further rural/urban divide?
