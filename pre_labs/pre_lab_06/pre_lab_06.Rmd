...
**Task** Load the five CSV files. Do they have the same column structure? **Answer** No, 2016 and 2018, but 2020 and 2022 both diverge from that structure. There is no file for 2024.
...
**Task** Run the following code to join Maryland population data and the 2020 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer** There are 24, which equal how many there should be--23 counties and Baltimore City. 
...
**Task** Create a new dataframe with population and voter data joined together. What does the output message below say about the join? **Answer** It joined the population and voter data together via the 'county' labels. 
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Answer** Prince George's has the highest Black adult population proportion, at around 60%, while also sporting the highest proportion of Democrats at nearly 80%. 
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Answer** The results are very different, with Howard, Montgomery and Baltimore Counties all ascending into the top 3. These three counties are also slightly less Democratic overall than the top 3 Black counties. 
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N? column? **Answer** The ZCTA5N column is a double vector column, meaning its counting real numbers, quantitatively. We need it to be a character column, a qualitative column. 
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N now? **Answer** It's now a character column.
