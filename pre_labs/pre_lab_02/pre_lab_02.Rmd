...
**Answer** The 'month' column is combining all dates with the same middle integer--the month section of the the whole date--and adding them together into a single row. That row is then assigned the value of the first of the given month, instead of just listing the month with the year, since the date column is set to the YMD pattern. 
...
**Answer** The 'general_22' dataset is being edited to add two new row via a mutate() function. One column, 'total_votes' is adding up all votes from a county. The second column, 'pct_moore', is taking the first column using its values to divide the votes for Wes Moore from each county, which gives us Moore's percentage of the total votes in each county.
...
**Answer** The counties (rows) have been been reordered, with the counties that most heavily votes for Wes Moore at the top and the counties that least heavily voted for Wes Moore at the bottom. This is by vote percentage, not total number of Moore votes.
...
**Answer** This change doesn't provide us any new information, but it could be useful if writing a story about where Wes Moore failed to gain a foothold in the gubernatorial race. The information is the same as the results from the previous task; it just may be easier to read for some. 
...
**Answer** 'str_to_lower' is similar but 'str_to_upper', except that it coverts to lowercase, not uppercase. 'str_to_title' converts just the first letter to uppercase, while the rest remains lowercase.  
...
**Answer** It's creating a new column titled 'state' via mutate() function. Then, using 'str_detect', the code is searching through the 'address' column for the names of states, including Maryland, California and others. If detected, the code adds a  value in the newly formed 'state' column--the abbreviation of the state detected. If it finds a Maryland address, MD is added, and so on. If no state name is found, the default value in 'state' will be NA. 
...
**Answer** It does, to a point. Of course, Maryland is at the top, with Virginia in tow. However, Washington state is an odd inclusion at #2. But, looking back at the code from the previous task, we can assume that the code is combining expenditures in Washington state and Washington DC, since the word 'Washington' is identified in both. To fix this, we would need to find a way to read the two separately. 
...
1.  What percent of expenditures in the state of Maryland have individual payees?

2.  Which region of the United States had the highest amount of expenditures?

3.  Which professors teach the most amount of classes with at least 100 seats?
