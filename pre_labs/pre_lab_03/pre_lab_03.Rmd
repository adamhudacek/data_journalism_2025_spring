...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer** No parsing issues found this time.
...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer** 'Vendor Name' and 'Date' are character, or vector/string, columns of data. 'Amount' is double data type, which means its a column of real numbers.
...
**Task** Run the following codeblock to group by the vendor's zip code and show any that are less than five characters in length. How many zip codes do not have five characters? What do you think explains those records? **Answer** There are 582 rows of Zip codes that come in under five characters. Perhaps a data scraper took the last four digits from a +4 zip code instead of the first five characters from that value. 
...
**Task** Run the following codeblock and describe below what each line is doing. Looking at the results, what jumps out at you as potentially problematic? **Answer** It's going into the md_grant_loans dataframe and looking at the Fiscal Year and Category columns. It's then summarizing the counts of category per year for each and arranging the data by ascending fiscal year. We're missing a row of loans from 2009, which indicates missing data. 
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Answer** I see a list of grantor names and amounts that begin with 60 dollars and range up to over 23,000 dollars. There are multiple low-dollar amount outliers in the beginning of the data. By the third page out of a 100, amount have crossed 1,000 dollars.
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does it change the headers? Answer below. **Answer** All the headers are turned into lowercase words and underscores replace spaces. 
...
**Task** Run the following codeblock to use str_to_upper() to make the source, grantee and description columns upper-cased. Describe what each line is doing. **Answer** We're creating a new dataframe from md_grants_loans called cleaned_md_grants_loans. In that new datafrane, we're cleaning names and renaming the grantor column to source, then mutating the source, grantee, and description columns into new versions of themselves with all uppercase row values. 
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **Answer** With 58 rows, there are 27 possible duplicate values. 
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer** The new column takes just the first 5 digits from the zip_code column vlaues and places them into a new column: zip5. 
