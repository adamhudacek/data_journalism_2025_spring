...
**Task** Run the following code and describe the dataframe it outputs. **Answer** It appears to be a list of medical professionals in Maryland who no longer have their medical license for a variety of reasons, some voluntary and some involuntary. 
...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **Answer** It shows each unique variables within the list and ignores all repititions. 
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **Answer** It creates a new dataframe with a single column, formed of every unique word from the original dataframe. There are over one million rows. 
...
**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **Answer** It shows the most frequent words that appear within the original dataframe. It is interesting, but it isn't particularly newsworthy. 
...
**Task** Run the following code and describe the results. Is it more interesting than before? **Answer** It is more interesting, but I still don't believe these results to be newsworthy, since context is removed from their usage.
...
**Task** Run the following code and describe the results. **Answer** The results appear largely similar between the two lists, with some placement changes amongst a nearly identical overall selection of terms.
...
**Task** Run the following code and describe the results. **Answer** We have a similar list as before, but instead of single words we have paired words, which creates extremely different results.
...
**Task** Run the following code and describe the results. **Answer** The results are identical to before, down to the percentages and list order. 
...
**Task** Run the following code and describe the results. **Answer** The top result remains the same, but now there are numbered participants that appear within the results. md code also appears lower than before.
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **Answer** 'Fine' has multiple meanings, some of them positive, some of them negative, like a fine as a punishment, so that could be incorrect. 'Appeal' and 'intimate' are also not certainly positive, as 'intimate' could mean medical professional committed a violation. Overall, a lack of context limits the effectiveness of thise list, since most words don't have a single, permanent definition.
